...
// #83. A pre-stream overload (429/503) must reach the editor as that status,
// with the upstream Retry-After intact, whether the request streamed or not --
// never a blind 200 event-stream carrying the error inside it. The header-peek
// worker learns the status before the sink is committed and answers it as a
// normal reply.
TestResult test_overload_headers_survive_streaming() {
    ...
    server.Post("/v1/chat/completions", [&](const httplib::Request& request,
        ...
        response.set_header("Retry-After",
                            response.status == 429 ? "7" : "Wed, 21 Oct 2037 07:28:00 GMT");
