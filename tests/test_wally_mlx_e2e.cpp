/**
 * @file test_wally_mlx_e2e.cpp
 * @brief In-process wally E2E coverage for the MLX backend contract.
 *
 * The production MLX runtime is Swift/MLX. This test installs the same C
 * callback table that the Swift runtime installs, then invokes the actual wally
 * command stack against a local MLX-style folder. That keeps the test offline
 * and fast while exercising wally parsing, bootstrap, backend registration,
...
TestResult test_wally_mlx_run_end_to_end() {
  ...
  const std::filesystem::path home = make_temp_dir("wally-mlx-home");
  const std::filesystem::path llm_dir = make_temp_dir("wally-mlx-llm");
  const std::filesystem::path vlm_dir = make_temp_dir("wally-mlx-vlm");
  const std::filesystem::path embedding_dir = make_temp_dir("wally-mlx-embed");
  const std::filesystem::path stt_dir = make_temp_dir("wally-mlx-stt");
  const std::filesystem::path tts_dir = make_temp_dir("wally-mlx-tts");
  ...
  if (!wally::wav::write_wav(input_wav.string(), pcm_samples.data(),
  ...
  wally::Bootstrapped bootstrapped;
  if (wally::bootstrap(options, &bootstrapped) != RAC_SUCCESS) {
  ...
  const bool tts_ok = run_cli_or_fail(
      {"wally", "--json", "--no-progress", "--home", home.string(), "tts",
...
int main(int argc, char **argv) {
  TestSuite suite("wally_mlx_e2e");
  suite.add("mlx_callback_bridge_all_slots", test_mlx_callback_bridge_all_slots);
  suite.add("wally_mlx_run_end_to_end", test_wally_mlx_run_end_to_end);
