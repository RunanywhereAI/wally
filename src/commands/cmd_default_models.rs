...
pub fn resolve_default_model(explicit_model: &str, no_color: bool) -> String {
    ...
    if explicit_model.is_empty() && !effective.is_empty() {
        ...
        out::status_line(&format!("{}model not provided, using default model {effective}{}", pal.blue, pal.reset));
    }
    effective
}
...
pub fn register_default_models(app: &mut App) {
    ...
    cmd.add_option("model", ValueType::Text, "Model id to save as the default (omit to show it)");
    ...
    cmd.callback(|p, _g| {
        ...
        match current.source {
            DefaultModelSource::Environment => out::result_line(&format!("{} (from WALLY_DEFAULT_MODEL)", current.id)),
            DefaultModelSource::File => out::result_line(&current.id),
            DefaultModelSource::BuiltIn => out::result_line(&format!("{} (built-in default)", current.id)),
            DefaultModelSource::None => out::status_line("no default model set; pass one to save it"),
        }
        0
    });
}
