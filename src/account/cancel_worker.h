...
/// comes from `bearer` at SEND time, not construction: a translator can renew
/// its token mid-session, and a cancel carrying the old one would be refused.
/// It is the session's own key and is never logged.
class CancelWorker {
   public:
