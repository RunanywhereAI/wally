...
WatchedResult PostWatched(UpstreamLease& lease, const WatchedCall& call) {
    ...
    request.response_handler = [&](const httplib::Response& response) {
        ...
        // Hand the headers to the caller before any body byte, so a streaming
        // caller can preserve a pre-stream status instead of a blind 200.
        if (call.on_headers) {
            call.on_headers(response);
        }
        if (state.abandoned.load()) {
