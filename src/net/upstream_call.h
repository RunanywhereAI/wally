...
struct WatchedCall {
    ...
    /// The response's headers, the moment they arrive and before any body byte:
    /// the status and any Retry-After, which a streaming caller needs to decide
    /// whether to commit a 200 stream or answer the pre-stream failure instead.
    /// Runs inside the response handler with the call's lock held; it must not
    /// block or touch the sink.
